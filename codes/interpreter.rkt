...
(define apply-env
  (lambda (initial-env search-var is_func)
    (letrec ((loop (lambda (env)
                     (cond ((null? env)
                           ...
                           ((and (pair? env) (func-var? (car env)))
                            (let ((func1 (car env))
                                  (saved-env (cdr env)))
                              (if (and is_func (eqv? search-var (func-var-p-name func1)))
...
(define value-of-operand
  (lambda (exp env is_func)
    (match exp
      [(var-exp var) (apply-env env var is_func)]
...
(define (eval parsed-exp env)
  ...
  (match parsed-exp
           ...
           (cond
            [condition [begin
                         ...
                         (let ([flag (subst exp env)])
                           (go (and (not (and (number? flag) (equal? (exact->inexact flag) 0.0))) flag (not end))))]]
            [else '()]))
     env)
    ...
    ((assign var exp)
     (set! env (extend-env var (value-of-operand exp env #f) env))
     env
     )
    ...
    ((assign-call var call)
     (set! env (extend-env var (value-of-operand call env #t) env))
     env
     )
...
(define (subst exp env)
  (match exp
    ...
    ((call-exp func1 args-lst)
     (let ((proc (let ((w (apply-env env func1 #t)))
                   (if (a-thunk? w)
                       (value-of-thunk w)
                       w))))
       (if (procedure? proc)
           [begin
             (let ((args (list)))
               (for ([i (length args-lst)])
                 (set! args (append args (list (value-of-operand (list-ref args-lst i) env #f)))))
               (apply-function proc args))]
           (error 'TypeError "~a object is not callable" (typeof proc)))))

    ((list_acc-exp var listmem)
     (define lst (let ((w (apply-env env var #f)))
                   (if (a-thunk? w)
                       (value-of-thunk w)
                       w)))
